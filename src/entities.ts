...
export class Entities extends EventEmitter {
    ...
    get gameRules(): GameRules {
        return this.getSingleton<CCSGameRulesProxy, GameRules>(
            "CCSGameRulesProxy",
        )
    }
    ...
    public getByUserId(userId: number): Player | null {
        const userInfoTable = this._demo.stringTables.findTableByName(
            "userinfo",
        )
        if (!userInfoTable) {
            return null
            ...
            while (true) {
                ...
                for (
                    ...
                    if (
                        ...
                        (priority === 64 &&
                            (prop.flags & SPROP_CHANGES_OFTEN) !== 0)
                    ) {
    ...
    private _readNewEntity(
        ...
        for (const update of updates) {
            ...
            const oldValue =
                table && varName in table ? table[varName] : undefined
    ...
    private _handleTempEntities(
        ...
        for (let i = 0; i < msg.numEntries; ++i) {
            ...
            if (entityBitBuffer.readOneBit()) {
                ...
                const updates = this._parseEntityUpdate(
                    entityBitBuffer,
                    lastClassId,
                )
